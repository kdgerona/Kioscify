...
function BrandDetailPageContent() {
  ...
  useEffect(() => {
    Promise.all([api.getBrandById(brandId), api.getMenus(brandId), api.getInventorySetups(brandId)])
      .then(([b, mns, setups]) => {
        setBrand(b);
        setMenus(mns);
  ...
  const loadTab = useCallback(
    async (tab: Tab) => {
      ...
      setTabLoading(true);
      try {
        setStores(await api.getStores(brandId));
      } catch {
  ...
  const handleSaveDelivery = async () => {
    ...
    try {
      ...
      toast.error(getErrorMessage(err, 'Failed to update delivery settings'));
    }
  };
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b gap-4">
              <div>
                ...
                <p className="text-xs text-gray-400 mt-0.5">
                  Each menu has its own independent categories, products, sizes, add-ons, preferences, and price tiers — fully isolated from other menus. Click a menu to open its workspace. Assign a menu to a store on the Stores tab — different stores can run different menus at the same time, so a nationwide rollout can happen store-by-store.
                </p>
              </div>
              {canWrite && (
                <button
            ...
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="divide-y divide-gray-200">
                {menus.length === 0 ? (
                  ...
                  <tr
                    key={menu.id}
                    onClick={() => router.push(`/brands/${brandId}/menus/${menu.id}`)}
            ...
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="divide-y divide-gray-200">
                {inventorySetups.length === 0 ? (
                  ...
                  <tr
                    key={setup.id}
                    onClick={() => router.push(`/brands/${brandId}/inventory-setups/${setup.id}`)}
                    className="hover:bg-gray-50 cursor-pointer"
                  >
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4">
                      {canWrite && editingStoreId === store.id ? (
                        <Select
                          ...
                          onValueChange={v => handleEditingStoreMenuChange(v === 'none' ? null : v)}
                        >
                    ...
                    <td className="px-6 py-4">
                      {canWrite && editingStoreId === store.id ? (
                        <Select
                          ...
                          onValueChange={v => setEditingStoreInventorySetupValue(v === 'none' ? null : v)}
                        >
                          <SelectTrigger className="w-40 h-8 text-xs">
                            <SelectValue placeholder="— Unassigned —" />
                          </SelectTrigger>
                      ...
                      {canWrite && editingStoreId === store.id ? (
                        ...
                        >
                          <SelectTrigger className="w-36 h-8 text-xs">
                            <SelectValue placeholder={editingStoreMenuValue ? '— None —' : '— No menu —'} />
                          </SelectTrigger>
                          <SelectContent>
    ...
    <Modal title={title} onClose={onClose}>
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex gap-3">
          ...
          <button type="submit" disabled={loading || !name.trim()} className="flex-1 py-2 text-white rounded-lg text-sm font-medium hover:brightness-90 disabled:opacity-50" style={{ backgroundColor: 'var(--company-primary, #ea580c)' }}>
            {loading ? 'Saving...' : 'Save'}
          </button>
        </div>
      </form>
