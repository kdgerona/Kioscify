...
import type { User, StorePrivileges } from '@/types';

const DEFAULT_FULL: StorePrivileges = {
  transactions: 'all',
  expenses: 'all',
  inventory: 'all',
  users: 'all',
...
export function EditPrivilegesModal({ open, onClose, onSave, storeId, user }: Props) {
  const [privileges, setPrivileges] = useState<StorePrivileges>(
    (user.storePrivileges as StorePrivileges | null) ?? DEFAULT_FULL
  );
  ...
  useEffect(() => {
    if (open) {
      setPrivileges((user.storePrivileges as StorePrivileges | null) ?? DEFAULT_FULL);
      setError(null);
    }
  }, [open, user]);
  ...
  const handleSave = async () => {
    ...
    setError(null);
    try {
      const updated = await api.updateStoreUserPrivileges(storeId, user.id, privileges);
      onSave(updated);
      onClose();
    } catch (err: unknown) {
