...
import type { StorePrivilegeLevel } from '@/types';

type Section = 'transactions' | 'expenses' | 'inventory' | 'users' | 'settings';

type Privileges = Record<Section, StorePrivilegeLevel>;
...
const LEVELS: { value: StorePrivilegeLevel; label: string }[] = [
...
export function PrivilegesGrid({ value, onChange, disabled }: Props) {
  const handleChange = (section: Section, level: StorePrivilegeLevel) => {
    if (disabled) return;
