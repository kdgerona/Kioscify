...
import { AlertTriangle, TrendingUp, Clock, Package, RefreshCw, CheckCircle2 } from 'lucide-react';
    ...
    <div className="p-8">
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-600 mt-2">
          Monitor critical inventory issues and take action before stockouts occur
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
        <button
          onClick={() => setSelectedType(selectedType === 'LOW_STOCK' ? null : 'LOW_STOCK')}
        ...
        >
          ...
          <p className="text-3xl font-bold text-gray-900">{alertsData?.alertsByType.LOW_STOCK || 0}</p>
        </button>

        <button
          onClick={() => setSelectedType(selectedType === 'USAGE_SPIKE' ? null : 'USAGE_SPIKE')}
          className={`bg-white rounded-xl shadow-sm p-6 border-2 transition hover:shadow-md text-left ${
            selectedType === 'USAGE_SPIKE' ? 'border-orange-500' : 'border-gray-200'
          }`}
        >
          <div className="flex items-center justify-between mb-4">
            ...
            {selectedType === 'USAGE_SPIKE' && (
              <span className="text-sm font-semibold text-orange-600">FILTERED</span>
            )}
          </div>
          <p className="text-gray-600 text-sm mb-1">Usage Spikes</p>
          <p className="text-3xl font-bold text-gray-900">{alertsData?.alertsByType.USAGE_SPIKE || 0}</p>
        </button>

        <button
          onClick={() => setSelectedType(selectedType === 'PROJECTED_STOCKOUT' ? null : 'PROJECTED_STOCKOUT')}
          className={`bg-white rounded-xl shadow-sm p-6 border-2 transition hover:shadow-md text-left ${
            selectedType === 'PROJECTED_STOCKOUT' ? 'border-red-500' : 'border-gray-200'
          }`}
        >
          <div className="flex items-center justify-between mb-4">
          ...
          <p className="text-3xl font-bold text-gray-900">{alertsData?.alertsByType.PROJECTED_STOCKOUT || 0}</p>
        </button>
      </div>
      ...
      {!alertsData || alertsData.totalAlerts === 0 ? (
        <div className="bg-white rounded-xl shadow-sm p-12 border border-gray-200 text-center">
          ...
          <p className="text-gray-600">No active alerts at this time. All inventory levels are healthy.</p>
        </div>
      ) : filteredAlerts.length === 0 ? (
        <div className="bg-white rounded-xl shadow-sm p-12 border border-gray-200 text-center">
          <CheckCircle2 className="w-16 h-16 text-green-500 mx-auto mb-4" />
          <p className="text-gray-900 text-xl font-bold mb-2">No {getAlertTypeLabel(selectedType!)} Alerts</p>
        ...
        <div className="space-y-4">
          {selectedType && (
            <div className="text-sm text-gray-600 mb-4">
              Showing {filteredAlerts.length} {getAlertTypeLabel(selectedType)} alert{filteredAlerts.length !== 1 ? 's' : ''}
            </div>
          )}
          {filteredAlerts.map((alert, index) => {
            ...
            return (
              <div key={`${alert.type}-${alert.itemId}-${index}`} className="bg-white rounded-xl shadow-sm p-6 border border-gray-200">
                {/* Alert Header */}
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-start space-x-4">
                    <div className="p-3 rounded-lg" style={{ backgroundColor: severityBgColor }}>
                    ...
                    <div>
                      <div className="flex items-center space-x-3 mb-1">
                        ...
                        <span
                          ...
                          style={{ backgroundColor: severityBgColor, color: severityColor }}
                        >
                          {alert.severity}
                        </span>
                      </div>
                      <p className="text-sm text-gray-600">
                        {alert.category.replace(/_/g, ' ')} • {alert.unit}
                      </p>
                    </div>
                  </div>
                ...
                <div className="bg-gray-50 rounded-lg p-4">
                  {alert.type === 'LOW_STOCK' && (
                    <div className="grid grid-cols-3 gap-4">
                      <div>
                        ...
                        <p className="text-2xl font-bold text-gray-900">{alert.currentQuantity}</p>
                      </div>
                      <div>
                        <p className="text-sm text-gray-600 mb-1">Minimum Level</p>
                        <p className="text-2xl font-bold text-gray-900">{alert.minStockLevel}</p>
                      </div>
                      <div>
                        <p className="text-sm text-gray-600 mb-1">Shortfall</p>
                        <p className="text-2xl font-bold text-red-600">-{alert.shortfall}</p>
                      </div>
                    </div>
                  )}
                  ...
                  {alert.type === 'USAGE_SPIKE' && (
                    <div className="grid grid-cols-3 gap-4">
                      <div>
                        <p className="text-sm text-gray-600 mb-1">Latest Consumption</p>
                        <p className="text-2xl font-bold text-gray-900">{alert.latestConsumption?.toFixed(1)}</p>
                      </div>
                      <div>
                        <p className="text-sm text-gray-600 mb-1">Average Consumption</p>
                        <p className="text-2xl font-bold text-gray-900">{alert.averageConsumption?.toFixed(1)}</p>
                      </div>
                      <div>
                        <p className="text-sm text-gray-600 mb-1">Increase</p>
                        <p className="text-2xl font-bold text-orange-600">+{alert.percentageIncrease?.toFixed(1)}%</p>
                      </div>
                    </div>
                  )}
                  ...
                  {alert.type === 'PROJECTED_STOCKOUT' && (
                    <div className="grid grid-cols-4 gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-2xl font-bold text-gray-900">{alert.avgDailyConsumption?.toFixed(1)}</p>
                      </div>
                      <div>
                      ...
                      <div>
                        <p className="text-sm text-gray-600 mb-1">Estimated Date</p>
                        <p className="text-lg font-bold text-red-600">
                          {alert.estimatedStockoutDate && formatDate(alert.estimatedStockoutDate)}
                        </p>
                      </div>
                    </div>
                  )}
