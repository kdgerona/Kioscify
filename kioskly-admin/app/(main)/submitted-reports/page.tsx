...
import { FileText, Search, Filter, Eye, X, RefreshCw, User } from 'lucide-react';
  ...
  return (
    <div className="p-8">
      <div className="mb-8">
        ...
        <p className="text-gray-600 mt-2">View all submitted daily sales reports</p>
      </div>
                  ...
                  <tr key={report.id} className="hover:bg-gray-50 transition">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`text-sm font-bold ${
                        report.summarySnapshot.grossProfit >= 0 ? 'text-green-600' : 'text-red-600'
                      }`}>
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-lg font-semibold text-gray-900">{selectedReport.reportDate}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-600 mb-1">Submitted At</p>
                  <p className="text-sm text-gray-900">{formatDateTime(selectedReport.submittedAt)}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-600 mb-1">Submitted By</p>
                  <p className="text-sm text-gray-900">
                    {selectedReport.user?.email || selectedReport.user?.username} ({selectedReport.user?.role})
                  </p>
                </div>
                <div>
                  <p className="text-sm text-gray-600 mb-1">Period Covered</p>
                  <p className="text-xs text-gray-700">
                    {new Date(selectedReport.periodStart).toLocaleString()} - {new Date(selectedReport.periodEnd).toLocaleString()}
                  </p>
                </div>
              </div>
              ...
              <div className="bg-gradient-to-br from-emerald-500 to-emerald-600 text-white p-6 rounded-xl">
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div>
                    ...
                    <p className="text-2xl font-bold">{formatCurrency(selectedReport.summarySnapshot.grossProfit)}</p>
                  </div>
                  <div>
                    <p className="text-emerald-100 text-sm">Profit Margin</p>
                    <p className="text-2xl font-bold">{selectedReport.summarySnapshot.profitMargin.toFixed(2)}%</p>
                  </div>
                  <div>
                    <p className="text-emerald-100 text-sm">Net Revenue</p>
                    <p className="text-2xl font-bold">{formatCurrency(selectedReport.summarySnapshot.netRevenue)}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-blue-50 p-6 rounded-xl border-2 border-blue-200">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="text-xl font-bold text-blue-900">
                      {formatCurrency(selectedReport.salesSnapshot.totalAmount)}
                    </p>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <p className="text-lg font-semibold text-blue-900">
                      {formatCurrency(selectedReport.salesSnapshot.averageTransaction)}
                    </p>
                  </div>
                  <div>
                ...
                <div className="mt-4 pt-4 border-t border-blue-300">
                  <p className="text-sm font-bold text-blue-900 mb-2">Payment Methods</p>
              ...
              <div className="bg-red-50 p-6 rounded-xl border-2 border-red-200">
                <h3 className="text-lg font-bold text-red-900 mb-4">Expenses</h3>
                <div className="grid grid-cols-3 gap-4">
                  <div>
                    <p className="text-sm text-red-700">Total Amount</p>
                    <p className="text-xl font-bold text-red-900">
                      {formatCurrency(selectedReport.expensesSnapshot.totalAmount)}
                    </p>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <p className="text-lg font-semibold text-red-900">
                      {formatCurrency(selectedReport.expensesSnapshot.averageExpense)}
                    </p>
                  </div>
                </div>
                ...
                <div className="mt-4 pt-4 border-t border-red-300">
                  <p className="text-sm font-bold text-red-900 mb-2">Categories</p>
                  <div className="grid grid-cols-2 gap-2">
                ...
                {selectedReport.expenses && selectedReport.expenses.length > 0 ? (
                  <div className="space-y-3 max-h-96 overflow-y-auto">
                    {selectedReport.expenses.map((expense) => (
                      <div key={expense.id} className="bg-white p-4 rounded-lg border border-gray-200">
                            ...
                            <div className="flex items-center gap-2 mt-1">
                              ...
                              {expense.user && (
                                <span className="text-xs text-gray-500">
                                  by {expense.user.username || expense.user.email}
                                </span>
                              )}
                            </div>
                            {expense.notes && (
              ...
              {selectedReport.notes && (
                <div className="bg-yellow-50 border border-yellow-200 p-4 rounded-lg">
                  ...
                  <p className="text-sm text-yellow-900">{selectedReport.notes}</p>
                </div>
              )}
            </div>
            ) : null}
